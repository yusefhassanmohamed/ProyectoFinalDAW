...
var numidExp = new RegExp(/^[0-9]{9}$/);
var numidValue = false;
const formulario = document.getElementById('formulario');
    ...
    $("#numero_identificacion").keyup(function (evento) {
        $("#numero_identificacion").val($("#numero_identificacion").val().toUpperCase());
        if(numidExp.test($("#numero_identificacion").val())){
            $("#error_numero_identificacion").addClass("oculto");
            $("#error_numero_identificacion").removeClass("activo");
            numidValue = true;
        }else{
            $("#error_numero_identificacion").addClass("activo");
            $("#error_numero_identificacion").removeClass("oculto");
            numidValue = false;
        }
    });
    ...
    formulario.addEventListener('submit', (e)=>{
        if(nombreValue && apellidoValue  && usernameValue && passwordValue && emailValue && telefonoValue && dniValue && numidValue){
